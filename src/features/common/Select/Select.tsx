...
export function Select({ selected, options, setSelected, className }: IProps) {
  ...
  return (
    <div className={`c-select ${className}`}>
      <div className="selected" onClick={() => setIsOptionsVisible(!isOptionsVisible)}>
        <div>{selected}</div>
        <svg
          version="1.1"
          viewBox="0 0 8 5"
          width="8px"
          height="5px"
          className={`${isOptionsVisible ? 'chevron--flip' : ''} chevron`}
          fill="currentColor"
        >
      ...
      {isOptionsVisible && <div className="options">{optionList}</div>}
      {/* </Transition> */}
    </div>
  );
}
